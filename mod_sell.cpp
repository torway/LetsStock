#include "mod_sell.h"
#include "ui_mod_sell.h"
#include "add_client.h"
#include "ui_add_client.h"
#include "mod_article.h"
#include "ui_mod_article.h"

mod_sell::mod_sell(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::mod_sell)
{
    ui->setupUi(this);
    this->setWindowTitle("LithoStock");
...
void mod_sell::actu_articles()
{
    ...
    while(categorie.next())
    {
        ...
        while(articles.next())
        {
            ...
            if(backGroundColor.red() > 128) art->setIcon(0, QIcon(":/ressources/images/edit_dark.png"));
            if(backGroundColor.red() <= 128) art->setIcon(0, QIcon(":/ressources/images/edit_light.png"));

            QIcon iconImg;
            if(!articles.value(8).toByteArray().isNull())
            {
                QByteArray data = articles.value(8).toByteArray();
...
void mod_sell::on_doubleSpinBox_valueChanged(double arg1)
{
    actu_montant();
}

void mod_sell::actu_montant()
{
    double amount = 0;
...
void mod_sell::on_pushButton_add_clicked()
{
...
void mod_sell::on_treeWidget_itemClicked(QTreeWidgetItem *item, int column)
{
    if(column == 0 && item->text(8) != "")
    {
        mod_article *mod = new mod_article;
    ...
    if(column == 1)
    {
        ourLabel *labelImg = new ourLabel;
...
void mod_sell::on_pushButton_addClient_clicked()
{
...
void mod_sell::on_comboBox_client_currentTextChanged(const QString &arg1)
{
    if(persoBefore == true)
    {
        for(int i = 0; i < ui->treeWidget->topLevelItemCount(); i++)
